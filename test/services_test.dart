...
void main() {
  ...
  setUp(() {
    SharedPreferences.setMockInitialValues({AppConstants.keyLoggedInUserId: 1});
  });
  ...
  group('NutritionService tests', () {
    ...
    test(
      'recordDismissedNutritionNotification marks nutrition alert dismissed today',
      () async {
        ...
        expect(
          prefs.getString('dismissed_nutrition_1_Lemak'),
          equals(todayDateStr),
        );
      },
    );
    ...
    test(
      'Smart Hysteresis resets tracking keys when nutrient returns to safe zone',
      () async {
        final prefs = await SharedPreferences.getInstance();
    ...
    test(
      'Verifies segregated notification ID ranges (40000..40005) for nutrition alerts',
      () {
  ...
  group('NotificationService tests', () {
    test('Verifies channel ids and notification service singleton', () {
      expect(
        ...
        equals('foodcura_nutrition_alerts'),
      );
      expect(
        NotificationService.mealChannelId,
        equals('foodcura_meal_reminders'),
      );
  ...
  group('ReminderService tests', () {
    ...
    test(
      'syncMealAlarms executes without error and updates settings',
      () async {
    ...
    test(
      'saveNotificationSettings updates preferences and clears meal cache',
      () async {
        final service = ReminderService();
    ...
    test(
      'changing meal time multiple times resets dismissal and prevents blocking',
      () async {
        final service = ReminderService();
        final prefs = await SharedPreferences.getInstance();
    ...
    test(
      'past meal time prevents retroactive popup while future time enables today schedule',
      () async {
        final service = ReminderService();
        final prefs = await SharedPreferences.getInstance();
        final now = DateTime.now();
        ...
        if (now.hour >= 1 && !(now.hour == 1 && now.minute == 0)) {
          expect(
            prefs.getString('last_notif_meal_1_Sarapan'),
            equals(todayDateStr),
          );
        }
    ...
    test(
      'daily notification key suppresses duplicate notifications on same day',
      () async {
        final prefs = await SharedPreferences.getInstance();
        final now = DateTime.now();
        final todayDateStr =
    ...
    test(
      'recordDismissedPantryNotification marks pantry item as dismissed for today',
      () async {
        final service = ReminderService();
        final prefs = await SharedPreferences.getInstance();
        final now = DateTime.now();
        ...
        expect(
          prefs.getString('dismissed_pantry_1_42') == tomorrowDateStr,
          isFalse,
        );
      },
    );
    ...
    test(
      'clearPantryNotificationTracking removes dismissal and daily notification keys',
      () async {
        final service = ReminderService();
        final prefs = await SharedPreferences.getInstance();
    ...
    test(
      'recordDismissedMealNotification marks meal reminder as dismissed for today',
      () async {
        final service = ReminderService();
        final prefs = await SharedPreferences.getInstance();
        final now = DateTime.now();
        ...
        expect(
          prefs.getString('dismissed_meal_1_Sarapan'),
          equals(todayDateStr),
        );
      },
    );
    ...
    test(
      'cancelAndDismissMealReminder dismisses meal reminder and cancels alarm',
      () async {
        final service = ReminderService();
        final prefs = await SharedPreferences.getInstance();
        final now = DateTime.now();
    ...
    test(
      'syncMealAlarms executes cleanly without error and respects dismissed flag',
      () async {
        final service = ReminderService();
        final prefs = await SharedPreferences.getInstance();
        final now = DateTime.now();
