...
* **Hardening Skema SQLite & Sinkronisasi Cloud Firestore**:
  ...
  - Memperbaiki sinkronisasi data master katalog makanan (`_ensureFoodCatalogSynced`) di [db_helper.dart](lib/database/db_helper.dart) sehingga database lokal tidak memuat ulang file asset JSON setiap kali aplikasi dibuka.
  - Memperkuat alur login dan *cross-device restore* di [auth_controller.dart](lib/controllers/auth_controller.dart) dengan auto-provisioning profil pengguna secara transparan.
* **Pembaruan Versi**:
  - Menaikkan versi aplikasi menjadi `version: 2.3.6+15` di [pubspec.yaml](pubspec.yaml) dan [app_constants.dart](lib/constants/app_constants.dart).

### **v2.3.5** — Notification Soft Delete Architecture & Cloud Sync Resilience
#### [Added]
